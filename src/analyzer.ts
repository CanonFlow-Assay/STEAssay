...
import type { Finding, Glossary, Position, Vocabulary } from "./model.js";
...
const wholeTerm = (term: string): RegExp =>
  new RegExp(
...
const parseMarkdown = (
  ...
  if (file.content.includes("\u0000")) {
    throw new MarkdownParseError(
      `${basename(file.relativePath)} contains a NUL byte; Markdown text cannot be observed safely.`,
    );
  }
  ...
  if (fence !== undefined) {
    throw new MarkdownParseError(
      `${basename(file.relativePath)} has an unclosed ${fence.repeat(3)} fenced code block.`,
    );
  }
  flush();
...
const makeFinding = (
  ...
  return {
    ...
    path,
    position,
    fingerprint: sha256(canonicalJson({ ruleId, path, position, message })),
    baseline: false,
  };
};
